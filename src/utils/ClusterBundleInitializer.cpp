#include "initializer/fht/ClusterBundleInitializer.h"

#include <algorithm>

ClusterBundleInitializer::ClusterBundleInitializer(const std::string& name) : 
    Initializer<FhtMethodTag>(name),
    PmtTypeChecker(PmtType::PMT_20INCH),
    m_dpc{},
    m_cmc{"ClusterMaxChargeInitializer", 1000, 0.0, 1000.0, 5.0, 2.0, 5.0, 10.0, 9418.0, 0.9, 1.5, 10000.0},
    m_dens_thold{1000.0},
    m_dist_thold{5000.0}
{}
...
bool ClusterBundleInitializer::operator()(const RecPmtTable& table) {
    unsigned int count = std::count_if(table.begin(), table.end(), [&](const RecPmtProp& pmt) { return check(pmt); });
